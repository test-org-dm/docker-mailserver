...
@test "checking relay hosts: default mapping is added from env vars for new user entry" {
  ...
  run_until_success_or_timeout 20 docker exec mail_with_relays grep -e domainzero.tld /etc/postfix/relayhost_map
...
@test "checking relay hosts: default mapping is added from env vars for new virtual user entry" {
  ...
  run_until_success_or_timeout 20 docker exec mail_with_relays grep -e domain2.tld /etc/postfix/relayhost_map
